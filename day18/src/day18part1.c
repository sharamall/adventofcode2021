...
int main(void) {
    ...
    int offset = 0;
    pair *root = read_pair(in->lines[0], &offset);
    root->parent = 0;
    print_pair(root);
    printf("\n");
    for (int i = 1; i < LENGTH; i++) {
        ...
        addition_pair->l = root;
        addition_pair->r = read_pair(in->lines[i], &offset);
        ...
        int result = 1;
        while (result) {
            process_for_explodes(root, 0);
            result = process_for_splits(root, 0);
        }
        print_pair(root);
        printf("\n");
    }
    print_pair(root);
    printf("\n");
    printf("magnitude of output %d\n", calculate_magnitude(root));
    free_pairs(root);
    dealloc_input(in);
    return 0;
}