...
int main(void) {
    ...
    int max_dist = 0;
    for (int i = 0; i < base.scanner_positions->size - 1; i++) {
        for (int k = i + 1; k < base.scanner_positions->size; k++) {
            vector3i *pos1 = base.scanner_positions->items[i];
            vector3i *pos2 = base.scanner_positions->items[k];
            int x_diff = abs(pos1->x - pos2->x);
            int y_diff = abs(pos1->y - pos2->y);
            int z_diff = abs(pos1->z - pos2->z);
            int dist = x_diff + y_diff + z_diff;
            if (dist > max_dist) {
                max_dist = dist;
            }
        }
    }
    printf("total beacons %d, max dist %d\n", in->beacons->size, max_dist);
